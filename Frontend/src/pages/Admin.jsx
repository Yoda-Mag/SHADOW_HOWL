...
const Admin = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await fetch(`${API_URL}/admin/users/search/${encodeURIComponent(query)}`, { headers });
      const data = await res.json();
      setUsers(data);
    } catch (err) {
  ...
  const toggleApproval = async (signalId) => {
    ...
    try {
      const res = await fetch(`${API_URL}/admin/signals/${signalId}/approve`, {
        method: 'PATCH',
        headers,
  ...
  const deleteSignal = async (id) => {
    ...
    try {
      await fetch(`${API_URL}/signals/delete/${id}`, { 
        method: 'DELETE', 
        headers 
      });
      fetchData();
